...
| 2026-09-24 | Each company's comparable history starts at its own first fiscal year on the current revenue basis, not a common date: Microsoft fiscal 2018, Alphabet fiscal 2017. A common overlapping window is used only for direct cross-company comparison. | Alphabet's FY2018 10-K says it adopted Topic 606 on 1 January 2017 with prior periods not adjusted. Microsoft's fiscal 2016-2017 revenue is on two bases. Forcing one start date would either discard comparable data or mix bases. See [`xbrl_mapping_googl.md`](xbrl_mapping_googl.md) and [`xbrl_mapping_msft.md`](xbrl_mapping_msft.md). |
| 2026-09-24 | Quarter consistency checks tolerate rounding: a reported three-month figure may differ from a year-to-date difference by up to 1.5 reporting units, and four quarters from the reported year by up to 2.5 units, where the unit (1,000,000 for figures in millions) is inferred from the values. Rounding-level differences are recorded in the check detail. Larger differences still warn or fail. | Alphabet's fiscal 2025 revenue quarters sum to 402,837m against a reported 402,836m. That is rounding in figures reported in millions, not a filing error, and exact equality would have failed correct data. |
| 2026-09-24 | Lease-adjusted FCF is computed only where a company discloses finance-lease cash flows. Quarters before the first disclosure are reported as "not disclosed", never zero. A company mapping finance-lease cash flows must state, from a reviewed filing, whether other infrastructure financing payments exist. A supplementary series, cash capex plus finance-lease assets obtained, is labelled an upper bound and is not part of the FCF definitions. | Alphabet's lease tags begin in 2022 and quarterly in fiscal 2024. Microsoft's finance-lease principal is small next to the assets it obtains under finance leases, so the two measures answer different questions. Alphabet's disclosed line is finance-lease financing cash flows including $1.1bn of 2025 prepayments, not pure principal. |
